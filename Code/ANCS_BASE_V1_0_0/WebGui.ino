...
<div class="navbar">
  <a href="/">Base</a>
  <a href="/camera1">Cam slot 1</a>
  <a href="/camera2">Cam slot 2</a>
  <a href="/camera3">Cam slot 3</a>
  <a href="/camera4">Cam slot 4</a>
  <a href="/camera5">Cam slot 5</a>
  <a href="/camera6">Cam slot 6</a>
</div>
)rawliteral";
...
void parseAndApplyParams(const String& request) {
  ...
  // WiFi Channel (base page)
  if (request.indexOf("wifiChannel=") >= 0) {
    int newChannel = getParamValue(request, "wifiChannel", 1, 11);  // legal 2.4 GHz channels
    if (wifiChannel != newChannel) {
      ...
      delay(10);
      ESP.restart();  // optional: restart to apply channel change
      return;
    }
  }
...
void serveBasePage(EthernetClient& client) {
  ...
  // ATEM IP + Status client.println("<div class='row'><span class='label'>ATEM IP: ");
  ...
  client.println("<input type='submit' id='submit' value='Set'></form>");

  // 🔹 Connection status message
  client.print("<div class='row'><span class='label'>State:</span> ");
  if (!AtemSwitcher.isConnected()) {
    client.println("<span class='status-error'>Cannot connect to ATEM, check IP settings</span>");
  } else {
    client.println("<span class='status-ok'>ATEM connection good</span>");
  }
  client.println("</div>");
  client.println("</div>");
  client.println("</div>");
  ...
  client.println("<div class='row'><small>Will also change channel on receivers currently in contact with Base.</small></div>");
  client.println("</div>");
  ...
  client.println("<div class='section'><div class='camera-section-title'>RSSI Levels (Closer to 0 is good)</div>");
  for (int i = 0; i < numberOfInputs; i++) {
...
void serveCameraPage(EthernetClient& client, int camIndex) {
  sendHtmlHeader(client, "Cam slot " + String(camIndex + 1));

  client.println("<div class='section'>");
  client.print("<div class='camera-section-title'>");
  client.print(SettingPhysicalInputNames[camIndex]);
  client.println("</div>");
