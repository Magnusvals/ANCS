...
#include <ArduinoJson.h>  // Install this via Library Manager
#include <TMCStepper.h>

#include <ESP32Encoder.h>

// Access Point credentials
const char* baseSSID = "ANCU-";  // Prefix for AP SSID
const char* apPassword = "12345678";
const int wifiChannel = 6;
#define WAIT_TIME 30000  // 30 seconds after no signal from Base to turn to standalone mode
...
void loop() {
  ...
  if (Settings.MotorCurrent != prevMotorCurrent) {
    setupStepperDriver();
    Serial.println("⚙️ Motor Current changed");
...
void joystickFocus(bool mode) {
  ...
  // Debug (optional)
  // Serial.print("localFocus: ");
  // Serial.println(localFocus, 4);
}
...
void onDataRecv(const esp_now_recv_info_t* recv_info, const uint8_t* data, int len) {
  ...
  if (payloadSize == sizeof(ccuVar)) {
    ...
    Serial.println("📥 CCU packet received.");
  } else if (payloadSize == sizeof(remoteSetting)) {
    // ✅ Settings packet
    memcpy(&Settings, data + 12, sizeof(remoteSetting));
