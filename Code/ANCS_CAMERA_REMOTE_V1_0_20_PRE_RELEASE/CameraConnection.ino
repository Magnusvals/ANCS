...
int getFocalLength() {
  ...
  if (client.connect(Settings.cameraIP, 80)) {
    ...
    if (error) {
      if (serialDebug) Serial.print("JSON parse error: ");
    ...
    if (doc.containsKey("focalLength")) {
      ...
      if (serialDebug) Serial.println("Key 'focalLength' not found in JSON");
    }
  } else {
    if (serialDebug) Serial.println("Connection to server failed.");
  }
...
bool getProductName() {
  if (client.connect(Settings.cameraIP, 80)) {
    ...
    if (!error && doc.containsKey("productName")) {
      ...
      strncpy(returnInfo.productName, name, sizeof(returnInfo.productName) - 1);
      returnInfo.productName[sizeof(returnInfo.productName) - 1] = '\0';  // ensure null termination
      prevCameraAnswer = currentMillis;
      return true;
    } else {
