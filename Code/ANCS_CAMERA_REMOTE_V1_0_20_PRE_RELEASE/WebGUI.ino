...
void handleSetIP() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (server.hasArg(argA)) {
      ...
      if (val >= 0 && val <= 255) {
        ...
        if (serialDebug) Serial.printf("Invalid selfIP[%d] value: %d\n", i, val);
      }
    }
    ...
    if (server.hasArg(argB)) {
      ...
      if (val >= 0 && val <= 255) {
        ...
        updated = true;
        if (serialDebug) Serial.print("Camera ");
      } else {
        if (serialDebug) Serial.printf("Invalid cameraIP[%d] value: %d\n", i, val);
      }
    }
  }
  ...
  if (updated) {
    ...
    if (serialDebug) Serial.println("No valid IP changes received.");
  }
}
...
void handleButtons() {
  ...
  switch (code) {
    ...
    case 80: Settings.useForce = false; break;
    case 81: Settings.useForce = true; break;
  }
  setStorage();  // store to memory
...
String buildHtmlPage() {
  ...
  // ===== CAMERA PRODUCT NAME SECTION =====
  html += "<div class=\"section\">";
  html += "<div class=\"row\">";
  html += "<span class=\"label\">Camera Model:</span>";

  if (strlen(returnInfo.productName) > 0) {
    html += returnInfo.productName;
  } else {
    html += "Unknown";
  }
  ...
  html += "'>";
  html += "<input type='submit' id='submit' value='Set'>";
  html += "<div><small>Recommended: 1, 6, or 11</small></div>";
  html += "</form>";
  html += "</div>";
  ...
  html += "<div class=\"section\">";
  html += "<div class=\"row\">";
  html += "<div class=\"label\">Base WL MAC:</div>";
  ...
  // --- Zoom Force Settings ---
  html += "<div class='section'><div class='camera-section-title'>Zoom Force Settings</div>";

  // Min Zoom Force
  html += "<div class='row'><form method='GET' action='/motorSettings'>";
  html += "<label>Min Zoom Force (0–2000):</label>";
  html += "<input type='number' name='minZoomForce' min='0' max='2000' value='";
  html += Settings.minZoomForce;
  html += "'>";
  html += "<input type='submit' id='submit' value='Set'>";
  html += "</form></div>";
  ...
  // Max Zoom Force
  html += "<div class='row'><form method='GET' action='/motorSettings'>";
  html += "<label>Max Zoom Force (0–2000):</label>";
  html += "<input type='number' name='maxZoomForce' min='0' max='2000' value='";
  html += Settings.maxZoomForce;
  html += "'>";
  html += "<input type='submit' id='submit' value='Set'>";
  html += "</form></div>";

  // Use Force toggle (same button logic as other ON/OFF controls)


  html += "<div class=\"row\">";
  html += "<div class='row'><label>Use Force:</label>";
  html += "<span id=\"useForce\">" + offOn[Settings.useForce] + "</span>";
  html += "<button class=\"button-on\" onclick=\"Buttons(81)\">ON</button>";
  html += "<button class=\"button-off\" onclick=\"Buttons(80)\">OFF</button>";
  html += "</div>";  // end row

  html += "</div>";  // end Zoom Force Settings section
  html += "</div>";  // end Zoom Force Settings section
  ...
  html += "<div class=\"row\">";
  html += "<div class=\"label\">Base WL MAC: ";
...
void handleMotorSettings() {
  ...
  if (server.hasArg("minZoomForce")) {
    int val = server.arg("minZoomForce").toInt();
    if (val >= 0 && val <= 2000) {
      Settings.minZoomForce = val;
      updated = true;
      Serial.printf("Updated minZoomForce: %d\n", val);
    } else {
      Serial.printf("Invalid minZoomForce: %d\n", val);
    }
  }

  if (server.hasArg("maxZoomForce")) {
    int val = server.arg("maxZoomForce").toInt();
    if (val >= 0 && val <= 2000) {
      Settings.maxZoomForce = val;
      updated = true;
      Serial.printf("Updated maxZoomForce: %d\n", val);
    } else {
      Serial.printf("Invalid maxZoomForce: %d\n", val);
    }
  }

  if (server.hasArg("useForce")) {
    int val = server.arg("useForce").toInt();
    if (val == 0 || val == 1) {
      Settings.useForce = val;
      updated = true;
      Serial.printf("Updated useForce: %s\n", val ? "ON" : "OFF");
    } else {
      Serial.printf("Invalid useForce: %d\n", val);
    }
  }
